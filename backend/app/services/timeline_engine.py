from backend.app.schemas.navigation import RouteRequest, TimelineRequest, TimelineResponse, TimelineStep
...
class TimelineEngine:
    ...
    def generate_timeline(self, request: TimelineRequest) -> TimelineResponse:
        ...
        self.hhmm_to_minutes(request.current_time)
        ...
        route_req = RouteRequest(start_node_id=gate_id, end_node_id=seat_node_id, accessibility_mode=acc_mode)
        ...
        # 1. Departure
        steps.append(TimelineStep(time=self.minutes_to_hhmm(departure_min), event_title="Depart for Stadium", description=f"Leave your current location. Estimated travel time is {travel_time} minutes via public transit.", type="departure"))
        ...
        steps.append(
            TimelineStep(
                time=self.minutes_to_hhmm(walking_start_min), event_title="Navigate to Seat", description=f"Follow route from gate to {request.ticket_seat_section}. Walking distance: {route_res.total_distance}m.", node_id=seat_node_id, type="seat"
            )
        )
        ...
        # 6. Kickoff
        steps.append(TimelineStep(time=self.minutes_to_hhmm(kickoff_min), event_title="Match Kickoff", description="FIFA World Cup 2026 match begins! Spain vs Argentina.", type="kickoff"))
        ...
        return TimelineResponse(steps=steps, estimated_seat_arrival=estimated_arrival_str, message=message)
